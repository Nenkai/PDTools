...
public class GT4GameData
{
    ...
    public void LoadFile(GT4Save gt4Save, string gameDataFilePath)
    {
        ...
        {
            throw new IOException($"Save folder access was denied, make sure that another program such as PCSX2 is not running.", e);
        }
    ...
    private static byte[] SerializePack(Span<byte> saveBuffer)
    {
        ...
        BinaryPrimitives.WriteInt32LittleEndian(packPtr[4..], -saveBuffer.Length);
        BinaryPrimitives.WriteUInt32LittleEndian(packPtr[8..], CRC32.crc32_0x77073096(saveBuffer, saveBuffer.Length));
        // Padding
    ...
    private static bool VerifyPackHeader(Memory<byte> packBuffer, out Memory<byte> saveBuffer)
    {
        ...
        int inputLength = -BinaryPrimitives.ReadInt32LittleEndian(packBuffer.Span[0x04..]);
        uint crc = BinaryPrimitives.ReadUInt32LittleEndian(packBuffer.Span[0x08..]);
