...
public class GameParameter
{
    ...
    public void ReadFromBuffer(ref BitStream reader)
    {
        ...
        if (magic != 0xE5E54F17 && magic != 0xE6E64F17)
            throw new Exception($"GameParameter did not match expected magic (0xE5E54F17 or 0xE6E64F17), got 0x{magic:X8}");
    ...
    {
        // We could use a STStuct utility for all this, but eh, effort, its just two fields
        using var ms = new MemoryStream();
        using var bs = new BinaryStream(ms, ByteConverter.Big);
        bs.WriteByte(0x0E); // PD/ST Struct version
        ...
        int boost_lv = -1, PenaltyLevelTypes penalty_level = PenaltyLevelTypes.OFF, List<int>? prize_table = null, bool one_make = false)
    {
        ...
        evnt.Reward.PrizeTable = prize_table ?? [];
        return gp;
    }
