...
public class CarModel1
{
    ...
    public static void Split(Stream inputStream, string outputDir)
    {
        ...
        using (var fs = new FileStream(Path.Combine(outputDir, "car_info.bin"), FileMode.Create))
        {
        ...
        using (var fs = new FileStream(Path.Combine(outputDir, "car_model_set.mdl"), FileMode.Create))
        {
