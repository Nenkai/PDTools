...
public class PS2ZIP
{
    ...
    /// </summary>
    public static async Task<bool> TryInflateAsync(Stream input, Stream output, int maxDecompressedSize = -1, bool skipMagic = false, CancellationToken ct = default)
    {
        if (!skipMagic && await input.ReadUInt32Async(cancellationToken: ct) != PS2ZIP_MAGIC)
            return false;

        int sizeComplement = -await input.ReadInt32Async(cancellationToken: ct);
        if (maxDecompressedSize != -1 && sizeComplement > maxDecompressedSize)
            return false;
        ...
        while (bytesLeft > 0 && (read = await ds.ReadAsync(buffer.AsMemory(0, Math.Min(buffer.Length, bytesLeft)), ct)) > 0)
        {
