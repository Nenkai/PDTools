...
namespace PDTools.Crypto
{
    public class MCipherGT5
    {
        ...
        {
            int size = SharedCrypto.EncryptUnit_Decrypt(buffer, length, crcSeed, 0.327032387256622, 0.858476877212524, false);
            if (size > -1)
    ...
    public class MCipherGT6
    {
        ...
        private static bool DecryptImpl(Span<byte> buffer, Span<byte> outBuffer, int length, uint crcSeed)
        {
            int size = SharedCrypto.EncryptUnit_Decrypt(buffer, length, crcSeed, 0.262591, 0.753459f, false);
            if (size > -1)
                buffer[8..].CopyTo(outBuffer);
            return size > -1;
        }
